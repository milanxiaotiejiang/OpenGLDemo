...
public class AirHockeyRenderer implements GLSurfaceView.Renderer {
    ...
    float[] tableVertices = {
            ...
            -0.5f, 0f, 0.5f, 0f, 0f, -0.25f, 0f, 0.25f,
...
　　另一个需要注意的是，uniform 被存储在硬件被称为常量存储，这是一种分配在硬件上的存储常量值的空间。因为这种存储需要的空间是固定的，在程序中这种uniform 的数量是受限的。这个限制能通过读gl_MaxVertexUniformVectors 和gl_MaxFragmentUniformVectors编译变量得出。（ 或者用GL_MAX_VERTEX_UNIFORM_VECTORS 或GL_MAX_FRAGMENT_UNIFORM_ VECTORS 为参数调用glGetIntegerv）OpenGL ES 2.0必须至少提供256 个顶点着色器uniform 和224个片段着色器uniform。
     */
        ...
        GLES20.glEnableVertexAttribArray(aPostionLocation);// Enable or disable a generic vertex attribute array
    }
...
函数说明：
      glClear（）函数的作用是用当前缓冲区清除值，也就是glClearColor或者glClearDepth、glClearIndex、glClearStencil、glClearAccum
      等函数所指定的值来清除指定的缓冲区，也可以使用glDrawBuffer一次清除多个颜色缓存。比如：
　　  glClearColor（0.0，0.0，0.0，0.0）;
　　  glClear（GL_COLOR_BUFFER_BIT）;
　　  第一条语句表示清除颜色设为黑色，第二条语句表示实际完成了把整个窗口清除为黑色的任务，glClear（）的唯一参数表示需要被清除的缓冲区。
     像素检验、裁剪检验、抖动和缓存的写屏蔽都会影响glClear的操作，其中，裁剪范围限制了清除的区域，而glClear命令还会忽略alpha函数、融合函数、逻辑操作、模板、
     纹理映射和z缓存；
     */
        // 1.清除屏幕和深度缓存
